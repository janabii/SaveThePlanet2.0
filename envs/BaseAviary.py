...
class BaseAviary(gym.Env):
        ...
        for _ in range(self.PYB_STEPS_PER_CTRL):
            ...
            for i in range (self.NUM_DRONES):
                if self.PHYSICS == Physics.PYB:
                    ...
                    # Apply fixed-wing physics if this is a fixed-wing drone
                    if self.DRONE_MODEL == DroneModel.FIXEDWING:
                        self._fixedWingPhysics(clipped_action[i, :], i)
    ...
    def _fixedWingPhysics(self,
                          rpm,
                          nth_drone
                          ):
        """Fixed-wing aerodynamics implementation.

        Adds lift, drag, and minimum speed constraints for fixed-wing flight.

        Parameters
        ----------
        rpm : ndarray
            (4)-shaped array of ints containing the RPMs values (used for thrust).
        nth_drone : int
            The ordinal number/position of the desired drone in list self.DRONE_IDS.
        """
        #### Get current state #####################################
        ...
        #### Calculate forward velocity and angle of attack ########
        # Forward direction in body frame
        forward_body = np.array([1, 0, 0])
        forward_world = np.dot(rotation, forward_body)
        
        # Forward velocity (projection onto forward direction)
        forward_vel = np.dot(vel, forward_world)
        ...
        if velocity_magnitude > 0.1:  # Avoid division by zero
            # Lift coefficient (simplified): increases with speed and pitch
            # CL = CL0 + CL_alpha * alpha
            pitch_angle = rpy[1]  # Pitch from roll-pitch-yaw
            angle_of_attack = pitch_angle  # Simplified: pitch ≈ AoA
            
            # Lift coefficient (simplified model)
            CL_0 = 0.3  # Base lift coefficient
            CL_alpha = 2.5  # Lift slope
            CL = CL_0 + CL_alpha * angle_of_attack
            
            # Stall at high angles (> ~15 degrees = 0.26 rad)
            if abs(angle_of_attack) > 0.26:
                CL *= 0.5  # Reduce lift in stall
            
            # Lift force magnitude (simplified): L = 0.5 * rho * V^2 * S * CL
            # Using empirical scaling for simulation
            wing_area = 0.15  # m^2 (approximate)
            air_density = 1.225  # kg/m^3
            lift_magnitude = 0.5 * air_density * velocity_magnitude**2 * wing_area * CL
            
            # Lift direction: perpendicular to velocity, in the up-pitch plane
            # Simplified: apply lift in world Z direction scaled by pitch
            velocity_dir = vel / velocity_magnitude
            # Up direction in world frame
            up_world = np.array([0, 0, 1])
            # Lift direction (perpendicular to velocity, upward component)
            lift_dir = up_world - np.dot(up_world, velocity_dir) * velocity_dir
            lift_dir_norm = np.linalg.norm(lift_dir)
            if lift_dir_norm > 0.01:
                lift_dir = lift_dir / lift_dir_norm
            else:
                lift_dir = up_world
            
            lift_force = lift_magnitude * lift_dir
            
            # Apply lift force to center of mass
            p.applyExternalForce(self.DRONE_IDS[nth_drone],
        ...
        #### Enhanced drag for fixed-wing ###########################
        # Fixed-wing has different drag characteristics
        # Drag increases significantly at low speeds (close to stall)
        CD_0 = 0.03  # Parasitic drag coefficient
        CD_induced = 0.05  # Induced drag (from lift)
        
        if velocity_magnitude > 0.1:
            # Total drag force
            drag_magnitude = 0.5 * air_density * velocity_magnitude**2 * wing_area * (CD_0 + CD_induced)
            drag_force = -drag_magnitude * (vel / velocity_magnitude)
        ...
        #### Minimum speed enforcement (anti-stall) #################
        min_speed = 8.0  # m/s
        if forward_vel < min_speed:
            # Apply forward thrust boost to prevent stall
            stall_correction = (min_speed - forward_vel) * 2.0
            stall_force = stall_correction * forward_world
            p.applyExternalForce(self.DRONE_IDS[nth_drone],
                                 4,
                                 forceObj=stall_force.tolist(),
                                 posObj=[0, 0, 0],
                                 flags=p.WORLD_FRAME,
                                 physicsClientId=self.CLIENT
                                 )
