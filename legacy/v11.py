...
# ======================================================================
# Quad close-up inspection camera
# ======================================================================
QUAD_CAM_W      = 1280       # high-res: 1280x960
QUAD_CAM_H      = 960
QUAD_CAM_FOV    = 60.0       # narrower FOV = more zoom on the target
QUAD_CAM_ALT    = 1.5        # metres above ground for the close-up hover
QUAD_DWELL_SECS = 2.0        # seconds the quad hovers still before shooting
CAMERA_FRAMES_DIR = "quad_frames"
...
def get_quad_camera_rgb(obs_state, cid):
    """
    Render a high-resolution close-up inspection photo from a quadcopter.

    The camera points straight down (nadir) so the cube fills the frame
    when the quad hovers at QUAD_CAM_ALT (~1.5 m above ground).
    Resolution: QUAD_CAM_W x QUAD_CAM_H (1280x960).
    """
    pos  = np.array(obs_state[0:3])
    quat = np.array(obs_state[3:7])

    rm  = p.getMatrixFromQuaternion(quat)
    fwd = np.array([rm[0], rm[3], rm[6]])   # body X = forward (used as image "up")
    up  = np.array([rm[2], rm[5], rm[8]])   # body Z = up

    # Camera eye slightly above drone centroid, looking straight down
    eye     = pos + 0.04 * up
    look_at = [eye[0], eye[1], 0.0]         # target = ground directly below

    view = p.computeViewMatrix(eye.tolist(), look_at, fwd.tolist())
    proj = p.computeProjectionMatrixFOV(
        QUAD_CAM_FOV, QUAD_CAM_W / QUAD_CAM_H, 0.05, 30.0)
...
def run(
    ...
    # Dwell (hover-and-shoot) state
    dwell_max    = int(QUAD_DWELL_SECS * control_freq_hz)  # steps to hover at site
    dwell_step   = 0       # current dwell counter
    site_arrived = False   # True once blue_ldr has reached the site WP
        ...
        while True:
            ...
            # ════════════════════════════════════════════════════════
                ...
                if fw_land_step >= fw_land_max:
                    ...
                    if not peaks:
                        ...
                        BLUE_WPS  = np.array([[x, y,       QUAD_CAM_ALT] for x, y in peaks], dtype=float)
                        GREEN_WPS = np.array([[x, y + 3.0, QUAD_CAM_ALT] for x, y in peaks], dtype=float)
            ...
            # ════════════════════════════════════════════════════════
                ...
                if len(BLUE_WPS) == 0:
                    ...
                    if not site_arrived:
                        # ── Phase A: fly to site ──
                        blue_ldr,  blue_reached = move_leader(
                            blue_ldr, BLUE_WPS[orange_idx], dt)
                        green_ldr, _            = move_leader(
                            green_ldr, GREEN_WPS[orange_idx], dt)

                        if blue_reached:
                            site_arrived = True
                            dwell_step   = 0
                            print(f"  [Squad] Arrived at site {site_n}/{len(BLUE_WPS)} "
                                  f"({wx:.1f}, {wy:.1f}) — hovering for photo…")
                    else:
                        # ── Phase B: hover still at site, count dwell steps ──
                        dwell_step += 1
                        # Keep leaders locked at the site waypoints (no drift)
                        blue_ldr[2]  = QUAD_CAM_ALT
                        green_ldr[2] = QUAD_CAM_ALT

                        # Take the photo at the midpoint of the dwell window
                        if dwell_step == dwell_max // 2:
                            ...
                            print(f"  [Squad] Photos captured at site {site_n} "
                                  f"({QUAD_CAM_W}x{QUAD_CAM_H} @ {QUAD_CAM_ALT:.1f}m alt) "
                                  f"→ {CAMERA_FRAMES_DIR}/")

                        if dwell_step >= dwell_max:
                            # ── Phase C: advance to next site ──
                            orange_idx  += 1
                            site_arrived = False
                            dwell_step   = 0
                            if orange_idx < len(BLUE_WPS):
                                nx, ny = detected_sites[orange_idx]
                                print(f"  [Squad] Moving to site {orange_idx+1} "
                                      f"({nx:.1f}, {ny:.1f})")
