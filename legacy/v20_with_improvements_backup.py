...
MISSION FLOW:
  ...
  4. SQUAD_GO     All 4 quads fly INDEPENDENTLY to their assigned sites:
                    ...
                    - capture 1024x1024 nadir photo → quad_frames/
...
# Optional synthetic 2D overlay hook (currently disabled in this file).
synth = None
synth_3d = True
...
NO_PROGRESS_DIST_EPS = 0.05
NO_PROGRESS_TIMEOUT_SECS = 12.0
...
QUAD_MATCH_RADIUS = 3.0
PRED_DEDUP_RADIUS_FW = 1.5
PRED_DEDUP_RADIUS_QUAD = 1.0
QUAD_RF_BURST_OFFSETS = (-2, 0, 2)  # dwell-step offsets around midpoint
GT_COORDS_CSV = os.path.join(_HERE, "ground_truth_waste.csv")
RUN_SUMMARY_JSON = os.path.join(_HERE, "run_summary.json")
...
def _roboflow_pixel_centers(rgb, *, confidence=60, overlap=30, allowed_classes=None):
    ...
    try:
        ...
        # Default: do not filter classes unless user explicitly asks via --rf_classes.
        ...
        for det in raw:
            ...
            if allowed is not None and (not cls or cls not in allowed):
                continue
...
def quad_camera_rgb(quad_pos, cid):
    """1024x1024 nadir shot from directly above a site."""
...
def dedup_pred_records(pred_records, min_dist):
    """Greedy NMS-style dedup in XY space using highest-score first."""
    if not pred_records:
        return []
    preds = sorted(pred_records, key=lambda x: x[2], reverse=True)
    kept = []
    for px, py, sc in preds:
        if all(math.hypot(px - kx, py - ky) >= min_dist for kx, ky, _ in kept):
...
def print_report(
    ...
    print(f"  Cumulative FW detections       : {total_px}")
    print(f"  Heatmap peak cell value       : {heatmap.grid.max():.2f}")
    print(f"  Ground-truth waste sites      : {NUM_TARGETS}")
    ...
    missed = max(0, NUM_TARGETS - len(matched_gt))
    ...
    def _clean_eval(ev):
        if ev is None:
            return None
        out = dict(ev)
        if "mean_err_m" in out and (
            out["mean_err_m"] is None or np.isnan(out["mean_err_m"])
        ):
            out["mean_err_m"] = None
        return out
    ...
    return {
        "hotspot": {
            ...
            "match_radius_m": float(MATCH_RADIUS),
        },
        "fw_eval": _clean_eval(fw_eval),
        "quad_eval": _clean_eval(quad_eval),
        "counters": {
            "fw_detection_frames": int(total_frames),
            "fw_detection_count": int(total_px),
            "photos_taken": int(photos_taken),
            "ground_truth_sites": int(NUM_TARGETS),
        },
    }


def save_run_summary(path, payload):
    try:
        with open(path, "w", encoding="utf-8") as f:
            json.dump(payload, f, indent=2)
        print(f"[Summary] Saved run summary -> {path}")
    except Exception as e:
        print(f"[Summary] Could not save run summary ({e})")
...
def run(
    ...
    waste_items = spawn_synthetic_waste_urdfs(terrain_id, tex_rgb, aabb_min, aabb_max, cid)
    ...
    waste_class_counts = {}
    for item in waste_items:
        waste_class_counts[item["name"]] = waste_class_counts.get(item["name"], 0) + 1
    ...
    quad_prev_wp_dist = [float("inf")] * NUM_DRONES
    quad_no_prog_wp = [0] * NUM_DRONES
    quad_rf_samples = [set() for _ in range(NUM_DRONES)]  # dwell steps already sampled
    ...
    quad_prev_home_dist = [float("inf")] * NUM_DRONES
    quad_no_prog_home = [0] * NUM_DRONES
    ...
    mode_str += f" | eval_mode={eval_mode}"
    banner(
        FW_SCANNING,
        ...
        while True:
                ...
                if fw_cam_ctr >= FW_CAM_SAMPLE:
                    ...
                    if _HAS_CV2 and FW_CAM_FRAMES_DIR:
                        ...
                        frame_path = os.path.join(
                            FW_CAM_FRAMES_DIR, f"fw_{fw_frame_idx:05d}.png"
                        )
            ...
            # ── FW_LAND ──────────────────────────────────────────────
                ...
                if fw_land_step >= fw_land_max:
                    ...
                    fw_eval = evaluate_detector(
                        dedup_pred_records(fw_pred_records, PRED_DEDUP_RADIUS_FW),
                        gt_xy,
                        MATCH_RADIUS,
                    )

                    if eval_mode == "independent_quad":
                        print(
                            "  [Eval] independent_quad mode: assigning quads to GT sites "
                            "(decoupled from FW hotspot quality)"
                        )
                        assignments, skipped = assign_peaks_to_quads(gt_xy, battery)
                    else:
                    ...
                    for qi in range(NUM_DRONES):
                        ...
                        quad_wp_idx[qi] = 0
                        quad_wp_age[qi] = 0
                        quad_prev_wp_dist[qi] = float("inf")
                        quad_no_prog_wp[qi] = 0
                        quad_rf_samples[qi] = set()
            ...
            # ── SQUAD_GO ─────────────────────────────────────────────
                ...
                for qi in range(NUM_DRONES):
                    ...
                    # ── flying toward current waypoint
                        ...
                        d_wp = math.hypot(obs[qi][0] - wp[0], obs[qi][1] - wp[1])
                        if d_wp < quad_prev_wp_dist[qi] - NO_PROGRESS_DIST_EPS:
                            quad_no_prog_wp[qi] = 0
                        else:
                            quad_no_prog_wp[qi] += 1
                        quad_prev_wp_dist[qi] = d_wp
                        ...
                        if here:
                            ...
                            quad_dwell[qi] = 0
                            quad_photo_taken[qi] = False
                        elif quad_no_prog_wp[qi] >= int(NO_PROGRESS_TIMEOUT_SECS * ctrl_hz):
                            wx, wy = assignments[qi][quad_wp_idx[qi]]
                            print(
                                f"  [Quad-{qi}/{QUAD_NAMES[qi]}] no-progress at "
                                f"({wx:.1f},{wy:.1f}) — forcing next site"
                            )
                            quad_wp_idx[qi] += 1
                            quad_wp_age[qi] = 0
                            quad_no_prog_wp[qi] = 0
                    ...
                    # ── dwelling overhead: hold XY, count steps, shoot once
                        ...
                        rf_steps = {
                            s
                            for s in (rf_mid + o for o in QUAD_RF_BURST_OFFSETS)
                            if 1 <= s <= dwell_max
                        }

                        # Run Roboflow on multiple dwell frames for stronger quad signal.
                        if (
                            _ROBOFLOW_MODEL is not None
                            and quad_dwell[qi] in rf_steps
                            and quad_dwell[qi] not in quad_rf_samples[qi]
                        ):
                            rgb_rf = quad_camera_rgb(obs[qi][0:3], cid)
                            q_pts, q_n, _q_cls, q_scores = detect_roboflow_quad_pts(
                                rgb_rf,
                                obs[qi][0:3],
                                confidence=max(10, rf_conf - 30),
                                overlap=rf_overlap,
                                allowed_classes=allowed_classes,
                            )
                            if q_n > 0:
                                for di, (qx, qy) in enumerate(q_pts):
                                    qscore = (
                                        float(q_scores[di])
                                        if q_scores and di < len(q_scores)
                                        else 0.0
                                    )
                                    quad_pred_records.append((qx, qy, qscore))
                                print(f"    [Quad-{qi}] RF burst -> {q_n} object(s)")
                            else:
                                print(f"    [Quad-{qi}] RF burst -> none")
                            quad_rf_samples[qi].add(quad_dwell[qi])
                ...
                if all(quad_done):
                    quad_eval = evaluate_detector(
                        dedup_pred_records(
                            quad_pred_records, PRED_DEDUP_RADIUS_QUAD
                        ),
                        gt_xy,
                        QUAD_MATCH_RADIUS,
                    )
                    ...
                    for i in range(NUM_DRONES):
                        leaders[i][2] = QUAD_CRUISE_ALT
                        quad_prev_home_dist[i] = float("inf")
                        quad_no_prog_home[i] = 0
            ...
            # ── SQUAD_BACK ───────────────────────────────────────────
                ...
                for i in range(NUM_DRONES):
                    ...
                    if d_home < quad_prev_home_dist[i] - NO_PROGRESS_DIST_EPS:
                        quad_no_prog_home[i] = 0
                    else:
                        quad_no_prog_home[i] += 1
                    quad_prev_home_dist[i] = d_home

                    if d_home > HOME_REACHED_DIST:
                        if quad_no_prog_home[i] < int(NO_PROGRESS_TIMEOUT_SECS * ctrl_hz):
                            all_home = False
                        if quad_no_prog_home[i] >= int(NO_PROGRESS_TIMEOUT_SECS * ctrl_hz):
                            print(
                                f"  [Quad-{i}/{QUAD_NAMES[i]}] no-progress on return — "
                                "marking as home for phase completion"
                            )
                            quad_no_prog_home[i] = 0
                            quad_prev_home_dist[i] = float("inf")
                            # Treat this quad as done with return phase.
                            pass
                    target_pos[i] = leaders[i].copy()
                ...
                if all_home:
                    ...
                    mission_state = QUAD_LAND
                    quad_land_step = 0
                    banner(QUAD_LAND, "Return timeout — descending at current positions")
                    ...
                    save_run_summary(
                        RUN_SUMMARY_JSON,
                        {
                            ...
                            "config": {
                                ...
                                "fw_alt": FW_ALT,
                                "quad_cam_alt": QUAD_CAM_ALT,
                                "quad_cam_fov": QUAD_CAM_FOV,
                                "quad_cam_w": QUAD_CAM_W,
                                "quad_cam_h": QUAD_CAM_H,
                            },
                            "waste": {
                                "count": int(NUM_TARGETS),
                                "class_counts": waste_class_counts,
                                "gt_csv": GT_COORDS_CSV,
                            },
                            "results": report_data,
                        },
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="mission",
        type=str,
        choices=["mission", "independent_quad"],
        help="Evaluation routing mode: mission (FW hotspots) or independent_quad (GT-site quad checks).",
    )
    parser.add_argument(
        "--seed",
        default=None,
        type=int,
        metavar="",
        help="Optional RNG seed for reproducible terrain/waste placement and behavior.",
    )
    parser.add_argument(
        "--no_pause",
        action="store_true",
        help="Exit without waiting for Enter at the end of the run.",
    )
