...
Start positions (diamond around home corner [-15,-15]):
  Quad 0 (Blue)   [-15.0, -12.5]   north
  Quad 1 (Green)  [-12.5, -15.0]   east
  Quad 2 (Red)    [-15.0, -17.5]   south
  Quad 3 (Purple) [-17.5, -15.0]   west
...
# ── 4 quad start positions in a diamond around the home corner ──────
#   north / east / south / west, each 2.5 m from HOME_XY
QUAD_START_XY = np.array([
    [HOME_XY[0],        HOME_XY[1] + 2.5],   # 0 Blue   — north
    [HOME_XY[0] + 2.5,  HOME_XY[1]      ],   # 1 Green  — east
    [HOME_XY[0],        HOME_XY[1] - 2.5],   # 2 Red    — south
    [HOME_XY[0] - 2.5,  HOME_XY[1]      ],   # 3 Purple — west
])
QUAD_HOVER = np.array(
...
def assign_peaks_to_quads(peaks):
    """
    Assign each peak to the quad whose START XY is nearest to that peak.
    Multiple peaks can be assigned to the same quad.

    Returns:
        assignments : list of 4 lists, each containing (wx, wy) tuples
                      assignments[i] = sites for quad i
    """
    assignments = [[] for _ in range(NUM_DRONES)]
    for wx, wy in peaks:
        dists = [math.hypot(wx - QUAD_START_XY[qi, 0],
                            wy - QUAD_START_XY[qi, 1])
                 for qi in range(NUM_DRONES)]
        nearest = int(np.argmin(dists))
        assignments[nearest].append((wx, wy))

    print("\n  [Assignment] Hotspot allocation:")
    for qi in range(NUM_DRONES):
        sites_str = ", ".join(f"({x:.1f},{y:.1f})" for x, y in assignments[qi])
        print(f"    Quad-{qi} ({QUAD_NAMES[qi]:6s}) <- {len(assignments[qi])} site(s): "
              f"{sites_str if sites_str else 'none'}")
    print()
    return assignments
            ...
            # ── QUAD_LAND ────────────────────────────────────────────
                ...
                if quad_land_step >= quad_land_max:
                    ...
                    print_report(peaks, heatmap, total_frames,
                                 total_px, photos_taken, assignments)
                    mission_state = DONE
                    break
