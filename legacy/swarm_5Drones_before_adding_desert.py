...
DEFAULT_NUM_DRONES = 5   # <-- five drones
...
# world limits and leader cap
WORLD_XY_LIMIT = 5.0
Z_MIN, Z_MAX = 0.2, 2.0
MAX_LEADER_SPEED = 1.2

# fixed W formation offsets relative to leader, world aligned
# Layout (top view):
#   4         L         3
#        2         5
W_OFFSETS = np.array([
    [ 0.0,  0.0],   # Leader (drone 0)
    [-1.0, -0.4],   # Drone 1, left-back
    [ 1.0, -0.4],   # Drone 2, right-back
    [-1.8,  0.6],   # Drone 3, left-front
    [ 1.8,  0.6],   # Drone 4, right-front
], dtype=float)
...
def update_follow_cam(target_pos_3d):
    ...
    # re-center on leader with F (keeps yaw/pitch/dist)
    # just ensures target is the leader, which we already pass in
...
def run(
    ...
    # safer staggered start to avoid early bumps (match W layout roughly)
    init_xyzs = np.array([
        [ 0.0,  0.0, 0.10],   # leader
        [-0.9, -0.4, 0.15],   # left-back
        [ 0.9, -0.4, 0.20],   # right-back
        [-1.7,  0.6, 0.25],   # left-front
        [ 1.7,  0.6, 0.30],   # right-front
    ], dtype=float)
    ...
    try:
        while True:
            ...
            # fixed W formation, world-aligned
            raw_targets = np.zeros((num_drones, 3), dtype=float)   # <-- fixed shape (N,3)
            ...
            # collision avoidance and bounds on targets
        ...
        env.close()
        logger.save()
        logger.save_as_csv("leader_fixed_W_followcam")
        if plot:
            logger.plot()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Keyboard leader with fixed W formation + FollowCam")
