"""
Save the Planet: Mixed UAV Fleet Waste Detection Mission
CAMERA-BASED DETECTION + HEATMAP + QUAD INSPECTION

MISSION FLOW:
  1. FW_SCANNING  Fixed-wing flies lawnmower grid at FW_ALT.
                  Every FW_CAM_SAMPLE steps a nadir camera frame is
                  captured, WHITE pixels are detected via HSV, and
                  back-projected into a 34x34 m heatmap grid.
                  Quads hover at base corner.

  2. FW_RETURN    Fixed-wing flies back to home corner.

  3. FW_LAND      Fixed-wing descends. Heatmap peaks extracted.
                  Heatmap tiles rendered on ground. PNG saved.

  4. SQUAD_GO     Both quads fly to each hotspot:
                    - fly to site at QUAD_CRUISE_ALT
                    - descend to QUAD_CAM_ALT (5 m) directly overhead
                    - hover still for QUAD_DWELL_SECS
                    - capture 1280x960 nadir photo, save to quad_frames/
                    - move to next site
                  Camera follows Blue quad.

  5. SQUAD_BACK   Both quads return to home corner.

  6. QUAD_LAND    Both quads descend and land.

  7. DONE         Full precision/recall report printed. Sim exits.

Controls:
  Arrows: rotate viewport   Z/X: zoom   F: free-cam   R: reset
  Ctrl+C: abort (partial report still printed)
"""

import os
import time
import math
import argparse
import numpy as np
import pybullet as p

try:
    import cv2
    _HAS_CV2 = True
except ImportError:
    cv2 = None
    _HAS_CV2 = False

from gym_pybullet_drones.utils.enums import DroneModel, Physics
from gym_pybullet_drones.envs.CtrlAviary import CtrlAviary
from gym_pybullet_drones.control.DSLPIDControl import DSLPIDControl
from gym_pybullet_drones.utils.Logger import Logger
from gym_pybullet_drones.utils.utils import sync, str2bool
from desert_utils import create_desert_terrain, spawn_desert_obstacles

# ======================================================================
# Mission states
# ======================================================================
FW_SCANNING = 0
FW_RETURN   = 1
FW_LAND     = 2
SQUAD_GO    = 3
SQUAD_BACK  = 4
QUAD_LAND   = 5
DONE        = 6
STATE_NAMES = {
    0: "FW_SCANNING",    1: "FW_RETURNING",
    2: "FW_LANDING",     3: "SQUAD_INSPECTION",
    4: "SQUAD_RETURNING",5: "QUAD_LANDING",
    6: "DONE",
}

# ======================================================================
# World geometry
# ======================================================================
HOME_XY = np.array([-15.0, -15.0])
FW_ALT  = 5.0
HOME_FW = np.array([HOME_XY[0], HOME_XY[1], FW_ALT])

QUAD_CRUISE_ALT = 4.0          # transit altitude for quads
QUAD_HOVER = np.array([
    [HOME_XY[0],       HOME_XY[1] + 2.5, QUAD_CRUISE_ALT],
    [HOME_XY[0] + 2.5, HOME_XY[1],       QUAD_CRUISE_ALT],
])

WORLD_LIMIT = 19.0
Z_MIN, Z_MAX = 0.2, 8.0        # safety clamp on target Z

# ======================================================================
# Lawnmower survey waypoints  (boustrophedon, 6 rows)
# ======================================================================
_ys  = [-16.0, -10.0, -4.0, 2.0, 8.0, 14.0]
_wps = []
for _i, _y in enumerate(_ys):
    _wps += [[-17.0, _y, FW_ALT], [17.0, _y, FW_ALT]] if _i % 2 == 0 \
        else [[17.0, _y, FW_ALT], [-17.0, _y, FW_ALT]]
FW_SCAN_WPS = np.array(_wps, dtype=float)

FW_SPEED      = 4.5
FW_WP_RADIUS  = 2.0
FW_LAND_SECS  = 4.0

# ======================================================================
# Quad navigation
# ======================================================================
SQUAD_SPD    = 2.5    # cruise speed (m/s)
LAND_SECS    = 4.0
MIN_SEP      = 2.2    # collision avoidance trigger distance
AVOID_GAIN   = 0.8
MAX_PUSH     = 1.0

# ======================================================================
# Waste cubes  (WHITE — unambiguous against orange/tan sand)
# ======================================================================
WASTE_POSITIONS = [
    (-15.0, -12.0, 0.0), (-15.0,  12.0, 0.0),
    (  0.0, -15.0, 0.0), (  0.0,  15.0, 0.0),
    ( 15.0, -12.0, 0.0), ( 15.0,  12.0, 0.0),
    ( -8.0,   0.0, 0.0), (  8.0,   0.0, 0.0),
]
CUBE_COLOR  = [1.0, 1.0, 1.0, 1.0]   # pure white
NUM_TARGETS = len(WASTE_POSITIONS)

# ======================================================================
# Drones
# ======================================================================
DRONE_CONFIGS = [
    {"model": DroneModel.CF2X, "color": [0.2, 0.2, 1.0, 1.0]},   # Blue
    {"model": DroneModel.CF2X, "color": [0.2, 0.8, 0.2, 1.0]},   # Green
]
NUM_DRONES = len(DRONE_CONFIGS)

# ======================================================================
# Simulation
# ======================================================================
DEFAULT_PHYSICS   = Physics("pyb")
DEFAULT_GUI       = True
DEFAULT_PLOT      = False
DEFAULT_SIM_HZ    = 240
DEFAULT_CTRL_HZ   = 30
DEFAULT_OUT_DIR   = "results"

# ======================================================================
# Viewport camera globals
# ======================================================================
_cam = dict(dist=20.0, yaw=45.0, pitch=-40.0, tx=0.0, ty=0.0, tz=0.0, free=False)
CAM_PITCH_MIN, CAM_PITCH_MAX = -89.0, -10.0

# ======================================================================
# Fixed-wing downward survey camera
# ======================================================================
FW_CAM_W      = 320
FW_CAM_H      = 240
FW_CAM_FOV    = 90.0     # ~10 m footprint at 5 m altitude
FW_CAM_OFFSET = 0.35     # m above FW centroid
FW_CAM_SAMPLE = 4        # capture every N control steps

# White HSV gate  (S<55, V>210)  — sand has S~100-140, rejected cleanly
WHITE_LO = np.array([  0,  0, 210], dtype=np.uint8)
WHITE_HI = np.array([179, 55, 255], dtype=np.uint8)
MIN_WHITE_PX  = 4
MAX_PROJ_SAMP = 200

# ======================================================================
# Quad inspection camera
# ======================================================================
QUAD_CAM_W      = 1280
QUAD_CAM_H      = 960
QUAD_CAM_FOV    = 75.0    # at 5 m alt: ~9 m footprint, fits full site
QUAD_CAM_ALT    = 5.0     # m above ground for the overhead hover
QUAD_DWELL_SECS = 2.5     # seconds to hover still before shooting
QUAD_FRAMES_DIR = "quad_frames"

# ======================================================================
# Heatmap
# ======================================================================
GRID_MIN      = -17.0
GRID_MAX      =  17.0
GRID_CELL     =  1.0
GRID_N        = int((GRID_MAX - GRID_MIN) / GRID_CELL)  # 34

HEAT_SPREAD   = 1.2     # Gaussian sigma in cells
PEAK_FRAC     = 0.28    # threshold as fraction of grid max
PEAK_MIN_HEAT = 5.0     # absolute minimum heat to be a peak candidate
PEAK_MIN_SEP  = 4.5     # min distance (m) between accepted peaks
HEATMAP_PNG   = "heatmap_detection.png"

# Report: a peak is a true positive if within this distance of a GT cube
MATCH_RADIUS  = 3.5


# ======================================================================
# Heatmap class
# ======================================================================
class WasteHeatmap:
    def __init__(self):
        self.grid    = np.zeros((GRID_N, GRID_N), dtype=np.float32)
        self.viz_ids = []

    def _wc(self, wx, wy):
        """World -> cell (clipped)."""
        cx = int(np.clip((wx - GRID_MIN) / GRID_CELL, 0, GRID_N - 1))
        cy = int(np.clip((wy - GRID_MIN) / GRID_CELL, 0, GRID_N - 1))
        return cx, cy

    def _cw(self, cx, cy):
        """Cell centre -> world."""
        return (GRID_MIN + (cx + 0.5) * GRID_CELL,
                GRID_MIN + (cy + 0.5) * GRID_CELL)

    def accumulate(self, world_pts, weight=1.0):
        spread = int(math.ceil(HEAT_SPREAD * 2)) + 1
        two_s2 = 2.0 * HEAT_SPREAD ** 2
        for wx, wy in world_pts:
            if not (GRID_MIN <= wx <= GRID_MAX and GRID_MIN <= wy <= GRID_MAX):
                continue
            cx, cy = self._wc(wx, wy)
            for dx in range(-spread, spread + 1):
                for dy in range(-spread, spread + 1):
                    ncx, ncy = cx + dx, cy + dy
                    if 0 <= ncx < GRID_N and 0 <= ncy < GRID_N:
                        self.grid[ncx, ncy] += weight * math.exp(
                            -(dx*dx + dy*dy) / two_s2)

    def find_peaks(self):
        gmax = float(self.grid.max())
        if gmax < PEAK_MIN_HEAT:
            return []
        thresh = max(PEAK_FRAC * gmax, PEAK_MIN_HEAT)
        cands  = []
        for cx in range(GRID_N):
            for cy in range(GRID_N):
                v = float(self.grid[cx, cy])
                if v >= thresh:
                    wx, wy = self._cw(cx, cy)
                    cands.append((v, wx, wy))
        cands.sort(reverse=True)
        peaks = []
        for _, wx, wy in cands:
            if all(math.hypot(wx - px, wy - py) >= PEAK_MIN_SEP
                   for px, py in peaks):
                peaks.append((wx, wy))
        return peaks

    def visualize(self, cid):
        if self.viz_ids:
            return
        gmax = float(self.grid.max()) or 1.0
        norm = self.grid / gmax
        count = 0
        for cx in range(GRID_N):
            for cy in range(GRID_N):
                v = float(norm[cx, cy])
                if v < 0.04:
                    continue
                wx, wy = self._cw(cx, cy)
                r = float(np.clip(v * 2.0,       0.0, 1.0))
                g = float(np.clip(v * 2.0 - 1.0, 0.0, 1.0))
                b = float(np.clip(v * 4.0 - 3.0, 0.0, 1.0))
                vs  = p.createVisualShape(
                    p.GEOM_BOX,
                    halfExtents=[GRID_CELL * 0.46, GRID_CELL * 0.46, 0.01],
                    rgbaColor=[r, g, b, 0.6], physicsClientId=cid)
                p.createMultiBody(0, -1, vs, [wx, wy, 0.05], physicsClientId=cid)
                count += 1
        self.viz_ids = [count]   # sentinel so we don't re-render
        print(f"  [Heatmap] {count} tiles rendered (black->red->yellow->white)")

    def mark_peaks(self, peaks, cid):
        for wx, wy in peaks:
            vs = p.createVisualShape(
                p.GEOM_CYLINDER, radius=0.75, length=0.06,
                rgbaColor=[1.0, 1.0, 0.0, 1.0], physicsClientId=cid)
            p.createMultiBody(0, -1, vs, [wx, wy, 0.18], physicsClientId=cid)

    def save_png(self, path=HEATMAP_PNG):
        gmax = float(self.grid.max())
        if gmax < 1e-6:
            print("  [Heatmap] Grid empty - nothing saved.")
            return
        if not _HAS_CV2:
            np.savetxt(path.replace(".png", ".csv"), self.grid, fmt="%.3f")
            print(f"  [Heatmap] CSV saved (install opencv-python for PNG)")
            return
        norm = (self.grid / gmax * 255.0).astype(np.uint8)
        img  = cv2.applyColorMap(norm.T[::-1, :], cv2.COLORMAP_HOT)
        h, w = img.shape[:2]
        for i in range(0, GRID_N + 1, 5):
            xi = int(i * w / GRID_N); yi = int(i * h / GRID_N)
            cv2.line(img, (xi, 0), (xi, h), (50, 50, 50), 1)
            cv2.line(img, (0, yi), (w,  yi), (50, 50, 50), 1)
            wv = int(GRID_MIN + i * GRID_CELL)
            cv2.putText(img, str(wv), (xi + 2, h - 4),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.28, (200, 200, 200), 1)
        cv2.imwrite(path, img)
        print(f"  [Heatmap] PNG saved -> {path}")


# ======================================================================
# Fixed-wing camera + white detection
# ======================================================================

def fw_camera_rgb(fw_pos, fw_dir, cid):
    """Nadir (straight-down) frame from the fixed-wing."""
    eye    = [fw_pos[0], fw_pos[1], fw_pos[2] + FW_CAM_OFFSET]
    target = [fw_pos[0], fw_pos[1], 0.0]
    fwd    = np.array([fw_dir[0], fw_dir[1], 0.0])
    n      = np.linalg.norm(fwd)
    fwd    = fwd / n if n > 1e-6 else np.array([1.0, 0.0, 0.0])
    view   = p.computeViewMatrix(eye, target, fwd.tolist())
    proj   = p.computeProjectionMatrixFOV(FW_CAM_FOV, FW_CAM_W/FW_CAM_H, 0.1, 60.0)
    raw    = p.getCameraImage(FW_CAM_W, FW_CAM_H, view, proj,
                              renderer=p.ER_TINY_RENDERER, physicsClientId=cid)
    return np.reshape(raw[2], (FW_CAM_H, FW_CAM_W, 4))[:, :, :3].astype(np.uint8)


def detect_white_pts(rgb, fw_pos, fw_dir):
    """
    Detect white pixels and back-project to world XY (flat-earth pinhole).
    Returns (world_pts list, pixel_count int).
    """
    if _HAS_CV2:
        hsv  = cv2.cvtColor(rgb, cv2.COLOR_RGB2HSV)
        mask = cv2.inRange(hsv, WHITE_LO, WHITE_HI)
    else:
        r = rgb[:, :, 0].astype(np.int16)
        g = rgb[:, :, 1].astype(np.int16)
        b = rgb[:, :, 2].astype(np.int16)
        mask = ((r > 210) & (g > 210) & (b > 210)
                & (np.abs(r - g) < 30) & (np.abs(g - b) < 30)
                & (np.abs(r - b) < 30)).astype(np.uint8) * 255

    n_px = int(np.count_nonzero(mask))
    if n_px < MIN_WHITE_PX:
        return [], n_px

    ys, xs = np.where(mask > 0)
    ns     = min(len(xs), MAX_PROJ_SAMP)
    idx    = np.random.choice(len(xs), ns, replace=False)

    cam_h    = fw_pos[2] + FW_CAM_OFFSET
    half_fwd = cam_h * math.tan(math.radians(FW_CAM_FOV) / 2)
    half_rgt = half_fwd * (FW_CAM_W / FW_CAM_H)

    yaw = math.atan2(fw_dir[1], fw_dir[0])
    fwd = np.array([ math.cos(yaw),  math.sin(yaw)])
    rgt = np.array([ math.sin(yaw), -math.cos(yaw)])

    pts = []
    for px, py in zip(xs[idx], ys[idx]):
        nr =  (px / FW_CAM_W - 0.5) * 2.0
        nu = -(py / FW_CAM_H - 0.5) * 2.0
        wx = fw_pos[0] + nu * half_fwd * fwd[0] + nr * half_rgt * rgt[0]
        wy = fw_pos[1] + nu * half_fwd * fwd[1] + nr * half_rgt * rgt[1]
        pts.append((wx, wy))
    return pts, n_px


# ======================================================================
# Quad inspection camera
# ======================================================================

def quad_camera_rgb(quad_pos, cid):
    """
    High-res nadir frame from a quad hovering directly above a site.
    quad_pos : [x, y, z] world position
    """
    eye    = [quad_pos[0], quad_pos[1], quad_pos[2]]
    target = [quad_pos[0], quad_pos[1], 0.0]
    up     = [1.0, 0.0, 0.0]   # image north = world +X
    view   = p.computeViewMatrix(eye, target, up)
    proj   = p.computeProjectionMatrixFOV(
        QUAD_CAM_FOV, QUAD_CAM_W / QUAD_CAM_H, 0.1, 30.0)
    raw    = p.getCameraImage(QUAD_CAM_W, QUAD_CAM_H, view, proj,
                              renderer=p.ER_TINY_RENDERER, physicsClientId=cid)
    return np.reshape(raw[2], (QUAD_CAM_H, QUAD_CAM_W, 4))[:, :, :3].astype(np.uint8)


def save_quad_photo(rgb, drone_name, site_idx):
    if not _HAS_CV2:
        return
    os.makedirs(QUAD_FRAMES_DIR, exist_ok=True)
    path = os.path.join(QUAD_FRAMES_DIR, f"site_{site_idx:02d}_{drone_name}.png")
    cv2.imwrite(path, cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR))
    print(f"    [Camera-{drone_name}] {QUAD_CAM_W}x{QUAD_CAM_H} saved -> {path}")


# ======================================================================
# Fixed-wing visual model
# ======================================================================

def _box(half, color, cid):
    c = p.createCollisionShape(p.GEOM_BOX, halfExtents=half, physicsClientId=cid)
    v = p.createVisualShape( p.GEOM_BOX, halfExtents=half, rgbaColor=color, physicsClientId=cid)
    return p.createMultiBody(0, c, v, [0, 0, -500], physicsClientId=cid)

def spawn_fw(cid):
    Y = [1.0, 0.85, 0.0, 1.0]; D = [0.8, 0.60, 0.0, 1.0]
    return [_box([0.55, 0.055, 0.055], Y, cid),
            _box([0.07, 0.85,  0.018], Y, cid),
            _box([0.06, 0.28,  0.012], D, cid),
            _box([0.05, 0.012, 0.200], D, cid)]

def place_fw(ids, pos, direction, cid):
    yaw = math.atan2(direction[1], direction[0])
    cy, sy = math.cos(yaw), math.sin(yaw)
    q = p.getQuaternionFromEuler([0, 0, yaw])
    for bid, (lx, ly, lz) in zip(ids,
            [(0,0,0),(0,0,0),(-0.52,0,0),(-0.52,0,0.16)]):
        p.resetBasePositionAndOrientation(
            bid,
            [pos[0] + cy*lx - sy*ly, pos[1] + sy*lx + cy*ly, pos[2] + lz],
            q, physicsClientId=cid)

def step_fw(ids, pos, dirn, target, dt, cid):
    """Advance FW one step. Returns (new_pos, new_dir, reached)."""
    diff = target - pos
    dist = math.hypot(diff[0], diff[1])
    if dist < FW_WP_RADIUS:
        place_fw(ids, pos, dirn, cid)
        return pos.copy(), dirn.copy(), True
    nd  = diff / (np.linalg.norm(diff) + 1e-9)
    np_ = pos + nd * FW_SPEED * dt
    np_[0] = np.clip(np_[0], -WORLD_LIMIT, WORLD_LIMIT)
    np_[1] = np.clip(np_[1], -WORLD_LIMIT, WORLD_LIMIT)
    np_[2] = target[2]
    place_fw(ids, np_, nd, cid)
    return np_, nd, False


# ======================================================================
# Quad software-leader navigation
# ======================================================================

def step_leader(ldr, wp, dt, speed=SQUAD_SPD):
    """Move leader one step toward wp. Returns (new_ldr, reached)."""
    diff = wp[:2] - ldr[:2]
    dist = np.linalg.norm(diff)
    if dist < 0.25:                      # tight: stop directly overhead
        out = ldr.copy(); out[2] = wp[2]
        return out, True
    nd  = diff / dist
    out = ldr.copy()
    out[:2] = ldr[:2] + nd * min(speed * dt, dist)
    out[0]  = np.clip(out[0], -WORLD_LIMIT, WORLD_LIMIT)
    out[1]  = np.clip(out[1], -WORLD_LIMIT, WORLD_LIMIT)
    out[2]  = wp[2]
    return out, False


# ======================================================================
# Viewport camera
# ======================================================================

def update_viewport(target, keys):
    c = _cam
    if ord('f') in keys and keys[ord('f')] & p.KEY_WAS_TRIGGERED:
        c['free'] = not c['free']
        if c['free']:
            c['tx'], c['ty'], c['tz'] = target
    for k, attr, delta in [
        (p.B3G_LEFT_ARROW,  'yaw',   -2.0),
        (p.B3G_RIGHT_ARROW, 'yaw',    2.0),
        (p.B3G_UP_ARROW,    'pitch',  1.5),
        (p.B3G_DOWN_ARROW,  'pitch', -1.5),
        (ord('z'),          'dist',  -0.5),
        (ord('x'),          'dist',   0.5),
    ]:
        if k in keys and keys[k] & p.KEY_IS_DOWN:
            c[attr] = np.clip(c[attr] + delta,
                              CAM_PITCH_MIN if attr == 'pitch' else -1e9,
                              CAM_PITCH_MAX if attr == 'pitch' else  1e9)
    if c['free']:
        cy_ = math.cos(math.radians(c['yaw']))
        sy_ = math.sin(math.radians(c['yaw']))
        if ord('w') in keys and keys[ord('w')] & p.KEY_IS_DOWN:
            c['tx'] += cy_; c['ty'] += sy_
        if ord('s') in keys and keys[ord('s')] & p.KEY_IS_DOWN:
            c['tx'] -= cy_; c['ty'] -= sy_
        if ord('q') in keys and keys[ord('q')] & p.KEY_IS_DOWN:
            c['tz'] += 0.5
        if ord('e') in keys and keys[ord('e')] & p.KEY_IS_DOWN:
            c['tz'] -= 0.5
    if ord('r') in keys and keys[ord('r')] & p.KEY_WAS_TRIGGERED:
        c.update(dist=20.0, yaw=45.0, pitch=-40.0, free=False)
    tgt = [c['tx'], c['ty'], c['tz']] if c['free'] else list(target)
    p.resetDebugVisualizerCamera(c['dist'], c['yaw'], c['pitch'], tgt)


# ======================================================================
# Report
# ======================================================================

def print_report(peaks, heatmap, total_frames, total_px, photos_taken):
    sep = "=" * 64
    print(f"\n{sep}")
    print("  MISSION COMPLETE — DETECTION REPORT")
    print(sep)
    print(f"  FW detection frames (white pixels seen)  : {total_frames}")
    print(f"  Cumulative white pixels accumulated      : {total_px}")
    print(f"  Heatmap peak cell value                  : {heatmap.grid.max():.2f}")
    print(f"  Ground-truth cube sites                  : {NUM_TARGETS}")
    print(f"  Heatmap hotspots found                   : {len(peaks)}")
    print(f"  Quad close-up photos saved               : {photos_taken}")
    print()

    true_pos   = 0
    false_pos  = 0
    matched_gt = set()

    if peaks:
        print("  Per-hotspot breakdown:")
        for i, (px, py) in enumerate(peaks):
            dists  = [math.hypot(px - gx, py - gy)
                      for gx, gy, _ in WASTE_POSITIONS]
            near_d = min(dists)
            near_i = int(np.argmin(dists))
            heat   = heatmap.grid[heatmap._wc(px, py)]
            if near_d <= MATCH_RADIUS and near_i not in matched_gt:
                matched_gt.add(near_i)
                true_pos += 1
                label = f"TRUE  POS  (GT#{near_i+1}, dist={near_d:.1f} m)"
            else:
                false_pos += 1
                label = f"FALSE POS  (nearest GT dist={near_d:.1f} m)"
            print(f"    Peak {i+1:2d}  ({px:6.1f}, {py:6.1f})  "
                  f"heat={heat:6.1f}  {label}")
    else:
        print("  No hotspots found.")

    missed    = NUM_TARGETS - len(matched_gt)
    precision = true_pos / max(len(peaks), 1) * 100
    recall    = true_pos / NUM_TARGETS * 100

    print()
    print(f"  True Positives  : {true_pos} / {NUM_TARGETS}")
    print(f"  False Positives : {false_pos}")
    print(f"  Missed (FN)     : {missed}")
    print(f"  Precision       : {precision:.1f}%")
    print(f"  Recall          : {recall:.1f}%")
    print(f"\n  Heatmap PNG     : {HEATMAP_PNG}")
    print(f"  Quad photos     : {QUAD_FRAMES_DIR}/")
    print(sep + "\n")


def banner(state, msg=""):
    print(f"\n{'='*60}\n  [{STATE_NAMES[state]}]  {msg}\n{'='*60}\n")


# ======================================================================
# MAIN
# ======================================================================

def run(
    physics   = DEFAULT_PHYSICS,
    gui       = DEFAULT_GUI,
    plot      = DEFAULT_PLOT,
    sim_hz    = DEFAULT_SIM_HZ,
    ctrl_hz   = DEFAULT_CTRL_HZ,
    out_dir   = DEFAULT_OUT_DIR,
):
    dt = 1.0 / ctrl_hz

    # ── Quad starting positions ───────────────────────────────────────
    init_xyzs = np.array([
        [QUAD_HOVER[0][0], QUAD_HOVER[0][1], QUAD_CRUISE_ALT],
        [QUAD_HOVER[1][0], QUAD_HOVER[1][1], QUAD_CRUISE_ALT],
    ], dtype=float)

    env = CtrlAviary(
        drone_model    = DroneModel.CF2X,
        num_drones     = NUM_DRONES,
        initial_xyzs   = init_xyzs,
        initial_rpys   = np.zeros((NUM_DRONES, 3)),
        physics        = physics,
        pyb_freq       = sim_hz,
        ctrl_freq      = ctrl_hz,
        gui            = gui,
        user_debug_gui = False,
    )

    cid = env.CLIENT
    if gui:
        p.configureDebugVisualizer(p.COV_ENABLE_GUI,          0, physicsClientId=cid)
        p.configureDebugVisualizer(p.COV_ENABLE_MOUSE_PICKING,1, physicsClientId=cid)

    # Sand ground
    p.changeVisualShape(env.PLANE_ID, -1, rgbaColor=[0,0,0,0], physicsClientId=cid)
    fv = p.createVisualShape(p.GEOM_BOX, halfExtents=[25, 25, 0.01],
                             rgbaColor=[0.87, 0.72, 0.53, 1.0], physicsClientId=cid)
    p.createMultiBody(0, -1, fv, [0, 0, -0.05], physicsClientId=cid)

    # Drone colours
    for i, cfg in enumerate(DRONE_CONFIGS):
        try:
            p.changeVisualShape(env.DRONE_IDS[i], -1,
                                rgbaColor=cfg["color"], physicsClientId=cid)
        except Exception:
            pass

    # Terrain + obstacles + waste
    print("[World] Building terrain...")
    create_desert_terrain(
        "assets/terrain_desert_dunes.png",
        "assets/desert_sand.png",
        (0.15, 0.15, 1.5))
    excl = list(WASTE_POSITIONS) + [
        (HOME_XY[0], HOME_XY[1], 0.0),
        (QUAD_HOVER[0][0], QUAD_HOVER[0][1], 0.0),
        (QUAD_HOVER[1][0], QUAD_HOVER[1][1], 0.0),
    ]
    spawn_desert_obstacles(12, 6, 35.0, excl, 5.0)

    for pos in WASTE_POSITIONS:
        cs = p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.30]*3, physicsClientId=cid)
        vs = p.createVisualShape(   p.GEOM_BOX, halfExtents=[0.30]*3,
                                    rgbaColor=CUBE_COLOR, physicsClientId=cid)
        p.createMultiBody(0, cs, vs, basePosition=pos, physicsClientId=cid)
    print(f"[World] {NUM_TARGETS} WHITE waste cubes spawned")

    # Fixed-wing
    fw_ids = spawn_fw(cid)
    fw_pos = HOME_FW.copy().astype(float)
    fw_dir = np.array([1.0, 0.0, 0.0], dtype=float)
    place_fw(fw_ids, fw_pos, fw_dir, cid)

    # Heatmap
    heatmap      = WasteHeatmap()
    fw_cam_ctr   = 0
    total_frames = 0
    total_px     = 0

    # Mission variables
    mission_state  = FW_SCANNING
    scan_idx       = 0
    fw_land_step   = 0
    fw_land_max    = int(FW_LAND_SECS * ctrl_hz)
    orange_idx     = 0
    quad_land_step = 0
    quad_land_max  = int(LAND_SECS * ctrl_hz)

    detected_sites = []
    BLUE_WPS  = np.empty((0, 3), dtype=float)
    GREEN_WPS = np.empty((0, 3), dtype=float)

    blue_ldr  = QUAD_HOVER[0].copy().astype(float)
    green_ldr = QUAD_HOVER[1].copy().astype(float)

    # Dwell state — quad hovers at site before shooting
    dwell_max    = int(QUAD_DWELL_SECS * ctrl_hz)
    dwell_step   = 0
    site_arrived = False
    photos_taken = 0

    # PID + logging
    ctrls      = [DSLPIDControl(drone_model=cfg["model"]) for cfg in DRONE_CONFIGS]
    action     = np.zeros((NUM_DRONES, 4))
    logger     = Logger(logging_freq_hz=ctrl_hz, num_drones=NUM_DRONES,
                        output_folder=out_dir)
    target_pos = init_xyzs.copy()

    t0   = time.time()
    step = 0

    banner(FW_SCANNING,
           f"White-cube detection | {len(FW_SCAN_WPS)} lines | "
           f"FOV={FW_CAM_FOV}deg | every {FW_CAM_SAMPLE} steps")
    print("[Fleet]  Yellow FW = scout | Blue = inspector | Green = wingman")
    print("[Controls] Arrows:rotate | Z/X:zoom | F:free | R:reset | Ctrl+C:abort\n")

    try:
        while True:
            keys = p.getKeyboardEvents(physicsClientId=cid)
            obs  = [env._getDroneStateVector(i) for i in range(NUM_DRONES)]

            # ── FW_SCANNING ──────────────────────────────────────────
            if mission_state == FW_SCANNING:

                fw_pos, fw_dir, reached = step_fw(
                    fw_ids, fw_pos, fw_dir, FW_SCAN_WPS[scan_idx], dt, cid)

                fw_cam_ctr += 1
                if fw_cam_ctr >= FW_CAM_SAMPLE:
                    fw_cam_ctr = 0
                    rgb = fw_camera_rgb(fw_pos, fw_dir, cid)
                    pts, n_px = detect_white_pts(rgb, fw_pos, fw_dir)
                    if n_px >= MIN_WHITE_PX:
                        total_frames += 1
                        total_px     += n_px
                        heatmap.accumulate(pts)
                        if total_frames % 10 == 1:
                            print(f"  [FW-cam] pos=({fw_pos[0]:5.1f},{fw_pos[1]:5.1f}) "
                                  f"white_px={n_px:4d}  heat_max={heatmap.grid.max():.1f}")

                if reached:
                    scan_idx += 1
                    print(f"  [FW] WP {scan_idx}/{len(FW_SCAN_WPS)} reached | "
                          f"heat_max={heatmap.grid.max():.1f}")
                    if scan_idx >= len(FW_SCAN_WPS):
                        mission_state = FW_RETURN
                        banner(FW_RETURN, "Scan done — returning home")

                target_pos[0] = QUAD_HOVER[0].copy()
                target_pos[1] = QUAD_HOVER[1].copy()

            # ── FW_RETURN ────────────────────────────────────────────
            elif mission_state == FW_RETURN:

                fw_pos, fw_dir, reached = step_fw(
                    fw_ids, fw_pos, fw_dir, HOME_FW, dt, cid)
                if reached:
                    mission_state = FW_LAND
                    fw_land_step  = 0
                    banner(FW_LAND, "Fixed-wing landing — analysing heatmap...")

                target_pos[0] = QUAD_HOVER[0].copy()
                target_pos[1] = QUAD_HOVER[1].copy()

            # ── FW_LAND ──────────────────────────────────────────────
            elif mission_state == FW_LAND:

                alpha     = min(1.0, fw_land_step / max(1, fw_land_max))
                fw_pos[2] = FW_ALT * (1.0 - alpha) + 0.15 * alpha
                place_fw(fw_ids, fw_pos, fw_dir, cid)
                fw_land_step += 1

                target_pos[0] = QUAD_HOVER[0].copy()
                target_pos[1] = QUAD_HOVER[1].copy()

                if fw_land_step >= fw_land_max:
                    peaks = heatmap.find_peaks()
                    heatmap.visualize(cid)
                    heatmap.mark_peaks(peaks, cid)
                    heatmap.save_png()

                    if not peaks:
                        print("  [Heatmap] No hotspots — skipping quad inspection")
                    else:
                        detected_sites = peaks
                        # Blue hovers exactly above peak; Green 3 m to the side
                        BLUE_WPS  = np.array(
                            [[x, y,       QUAD_CAM_ALT] for x, y in peaks], dtype=float)
                        GREEN_WPS = np.array(
                            [[x + 3.0, y, QUAD_CAM_ALT] for x, y in peaks], dtype=float)
                        print(f"  [Heatmap] {len(peaks)} hotspot(s) found — "
                              f"quads launching")

                    orange_idx   = 0
                    site_arrived = False
                    dwell_step   = 0
                    mission_state = SQUAD_GO
                    banner(SQUAD_GO,
                           f"Camera -> Blue | {len(BLUE_WPS)} site(s) | "
                           f"hover alt={QUAD_CAM_ALT}m dwell={QUAD_DWELL_SECS}s")

            # ── SQUAD_GO ─────────────────────────────────────────────
            elif mission_state == SQUAD_GO:

                if len(BLUE_WPS) == 0:
                    mission_state = SQUAD_BACK
                    banner(SQUAD_BACK, "No sites — returning")

                elif orange_idx < len(BLUE_WPS):
                    wx, wy   = detected_sites[orange_idx]
                    site_num = orange_idx + 1

                    if not site_arrived:
                        # ── fly to site ──────────────────────────────
                        blue_ldr,  b_here = step_leader(
                            blue_ldr,  BLUE_WPS[orange_idx],  dt)
                        green_ldr, _      = step_leader(
                            green_ldr, GREEN_WPS[orange_idx], dt)

                        if b_here:
                            site_arrived = True
                            dwell_step   = 0
                            print(f"  [Squad] Site {site_num}/{len(BLUE_WPS)} "
                                  f"({wx:.1f},{wy:.1f}) — overhead, hovering…")

                    else:
                        # ── dwell: hold position ─────────────────────
                        blue_ldr[2]  = QUAD_CAM_ALT
                        green_ldr[2] = QUAD_CAM_ALT
                        dwell_step  += 1

                        # shoot at midpoint of dwell (PID fully settled)
                        if dwell_step == dwell_max // 2:
                            b_rgb = quad_camera_rgb(obs[0][0:3], cid)
                            g_rgb = quad_camera_rgb(obs[1][0:3], cid)
                            save_quad_photo(b_rgb, "blue",  site_num)
                            save_quad_photo(g_rgb, "green", site_num)
                            photos_taken += 2
                            print(f"  [Squad] Photos taken at site {site_num} "
                                  f"({QUAD_CAM_W}x{QUAD_CAM_H} nadir @ {QUAD_CAM_ALT}m)")

                        if dwell_step >= dwell_max:
                            # advance to next site
                            orange_idx  += 1
                            site_arrived = False
                            dwell_step   = 0

                else:
                    mission_state = SQUAD_BACK
                    banner(SQUAD_BACK, "All sites inspected — returning")

                target_pos[0] = blue_ldr.copy()
                target_pos[1] = green_ldr.copy()

            # ── SQUAD_BACK ───────────────────────────────────────────
            elif mission_state == SQUAD_BACK:

                blue_ldr,  b_home = step_leader(blue_ldr,  QUAD_HOVER[0], dt)
                green_ldr, g_home = step_leader(green_ldr, QUAD_HOVER[1], dt)
                target_pos[0] = blue_ldr.copy()
                target_pos[1] = green_ldr.copy()

                if b_home and g_home:
                    mission_state  = QUAD_LAND
                    quad_land_step = 0
                    banner(QUAD_LAND, "Both quads descending")

            # ── QUAD_LAND ────────────────────────────────────────────
            elif mission_state == QUAD_LAND:

                alpha = min(1.0, quad_land_step / max(1, quad_land_max))
                z_now = QUAD_CRUISE_ALT * (1.0 - alpha) + 0.15 * alpha
                for i in range(NUM_DRONES):
                    target_pos[i] = np.array(
                        [QUAD_HOVER[i][0], QUAD_HOVER[i][1], z_now])
                quad_land_step += 1

                if quad_land_step >= quad_land_max:
                    peaks = heatmap.find_peaks()
                    print_report(peaks, heatmap, total_frames, total_px, photos_taken)
                    mission_state = DONE
                    break

            # ── DONE ─────────────────────────────────────────────────
            elif mission_state == DONE:
                break

            # ── Always refresh FW visual ──────────────────────────────
            place_fw(fw_ids, fw_pos, fw_dir, cid)

            # ── Quad collision avoidance ──────────────────────────────
            for i in range(NUM_DRONES):
                push = np.zeros(2)
                for j in range(NUM_DRONES):
                    if j == i: continue
                    dx = obs[i][0] - obs[j][0]
                    dy = obs[i][1] - obs[j][1]
                    d  = math.hypot(dx, dy)
                    if 1e-6 < d < MIN_SEP:
                        push += AVOID_GAIN * (MIN_SEP - d) / d * np.array([dx, dy])
                n_ = np.linalg.norm(push)
                if n_ > MAX_PUSH: push *= MAX_PUSH / n_
                target_pos[i, 0:2] = np.clip(
                    target_pos[i, 0:2] + push, -WORLD_LIMIT, WORLD_LIMIT)
                target_pos[i, 2]   = np.clip(target_pos[i, 2], Z_MIN, Z_MAX)

            # ── PID control ───────────────────────────────────────────
            for i, ctrl in enumerate(ctrls):
                raw = ctrl.computeControlFromState(
                    control_timestep = dt,
                    state            = obs[i],
                    target_pos       = target_pos[i],
                    target_rpy       = np.zeros(3),
                    target_vel       = np.zeros(3),
                    target_rpy_rates = np.zeros(3),
                )
                action[i, :] = raw[0]
                logger.log(drone=i, timestamp=step * dt, state=obs[i])

            env.step(action)

            # ── Viewport: follow FW during scan, Blue quad after ──────
            cam_tgt = (fw_pos.tolist()
                       if mission_state in (FW_SCANNING, FW_RETURN, FW_LAND)
                       else list(obs[0][0:3]))
            update_viewport(cam_tgt, keys)
            env.render()
            if gui: sync(step, t0, env.CTRL_TIMESTEP)
            step += 1

    except KeyboardInterrupt:
        print("\n[Abort] Ctrl+C — partial report:")
        peaks = heatmap.find_peaks()
        heatmap.save_png()
        print_report(peaks, heatmap, total_frames, total_px, photos_taken)

    finally:
        env.close()
        logger.save()
        logger.save_as_csv("mixed_fleet_mission")
        if plot: logger.plot()
        print("[Mission] Logs saved.")
        input("Press Enter to exit...")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--gui",  default=DEFAULT_GUI,  type=str2bool, metavar="")
    parser.add_argument("--plot", default=DEFAULT_PLOT, type=str2bool, metavar="")
    args = parser.parse_args()
    run(gui=args.gui, plot=args.plot)