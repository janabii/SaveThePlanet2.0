...
def prepare_terrain_texture(texture_path):
    ...
    out_path = os.path.join(os.path.dirname(texture_path), "terrain_texture_autofill.png")
    try:
...
def update_best_site_detection(best_entry, q_pts, q_scores, site_xy):
    ...
    for di, (qx, qy) in enumerate(q_pts):
        qscore = (
            float(q_scores[di]) if q_scores and di < len(q_scores) else 0.0
        )
...
def run(
    ...
    quad_site_best = [None] * NUM_DRONES  # best quad detection for current site
            ...
            # ── SQUAD_GO ─────────────────────────────────────────────
                ...
                for qi in range(NUM_DRONES):
                    ...
                    # ── dwelling overhead: hold XY, count steps, shoot once
                        ...
                        if _ROBOFLOW_MODEL is not None and quad_dwell[qi] in sample_steps:
                            rgb_s = quad_camera_rgb(obs[qi][0:3], cid)
                            q_pts, q_n, _q_cls, q_scores = detect_roboflow_quad_pts(
                                rgb_s,
                                obs[qi][0:3],
                                confidence=max(5, rf_conf - 35),
                                overlap=rf_overlap,
                                allowed_classes=allowed_classes,
                            )
                            if q_n > 0:
