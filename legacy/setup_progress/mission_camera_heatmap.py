...
def build_odm_terrain(heightmap_path, texture_path, arena_m, max_height_m, cid):
    ...
    # PyBullet places the heightfield corner at origin by default;
    # shift by half the arena size so it is centred.
    mid_z = (hm_arr.min() + hm_arr.max()) / 2.0
    body = p.createMultiBody(
        baseMass        = 0,
        baseCollisionShapeIndex = shape,
        baseVisualShapeIndex    = shape,
        basePosition    = [0.0, 0.0, mid_z],
        physicsClientId = cid,
    )
...
FW_CAM_OFFSET = 0.35
FW_CAM_SAMPLE  = 4
FW_CAM_FRAMES_DIR = "fw_frames"   # set to "" to disable FW frame saving
...
def run(
    ...
    # Asset paths should be resolved relative to this script so the example
    # can be run from any working directory.  In the original version the
    # hard-coded "assets/..." paths rely on the current working directory
    # being the examples folder, which is often not the case and leads to
    # FileNotFoundError (see issue reported by user).
    base_dir = os.path.dirname(__file__)
    hm_path  = os.path.normpath(
        os.path.join(base_dir, "..", "assets", "odm_heightmap_128.png"))
    tex_path = os.path.normpath(
        os.path.join(base_dir, "..", "assets", "odm_texture_512.png"))
    if not os.path.isfile(hm_path):
        raise FileNotFoundError(
            f"Heightmap file not found: {hm_path}\n"
            "Make sure you run the example from the repository root or install"
            " the required assets, or edit the path accordingly.")
    if not os.path.isfile(tex_path):
        print(f"[Warning] Texture file not found: {tex_path} "
              "— terrain will be grey")
