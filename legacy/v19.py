...
MISSION FLOW:
  1. FW_SCANNING  Fixed-wing lawnmower grid. Nadir camera detects CYAN
...
def detect_cyan_pts(rgb, fw_pos, fw_dir):
    ...
    if _HAS_CV2:
        ...
        mask = ((r < 80) & (g > 160) & (b > 160)
                & (np.abs(g - b) < 40)).astype(np.uint8) * 255
...
def run(
    ...
    hm_path = os.path.normpath(os.path.join(_HERE, "..", "assets", \
                                            "odm_heightmap_128.png"))
    tex_path = os.path.normpath(os.path.join(_HERE, "..", "assets", \
                                             "odm_texture_512.png"))
    ...
    print(f"[World] {NUM_TARGETS} CYAN waste cubes | {NUM_DRONES} quads")
        ...
        while True:
                ...
                if fw_cam_ctr >= FW_CAM_SAMPLE:
                    ...
                    if n_px >= MIN_CYAN_PX:
                        ...
                        if step % (FW_CAM_SAMPLE * 75) == 0:
                            print(f"  [FW-cam] ({fw_pos[0]:5.1f},{fw_pos[1]:5.1f}) "
                                  f"cyan_px={n_px} (below threshold)")
            ...
            # ── SQUAD_BACK ───────────────────────────────────────────
                ...
                for i in range(NUM_DRONES):
                    ...
                    leaders[i], _ldr_home = step_leader(
