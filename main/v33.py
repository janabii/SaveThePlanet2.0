...
try:
    ...
    print("[Roboflow] Plastic waste model loaded (save-the-planet_duplicate v7)")
...
class WasteHeatmap:
    ...
    def mark_peaks(self, peaks, cid):
        for wx, wy in peaks:
            ...
            bid = p.createMultiBody(0, -1, vs, [wx, wy, 0.8], physicsClientId=cid)
            self.peak_ids.append(bid)
    ...
    def mark_assignment(self, assignments, cid):
        ...
        for qi, sites in enumerate(assignments):
            for wx, wy in sites:
                ...
                bid = p.createMultiBody(0, -1, vs, [wx, wy, 1.1], physicsClientId=cid)
                self.assignment_ids.append(bid)

    def remove_viz(self, cid):
        """Remove all heatmap visual bodies from the simulation."""
        for bid in self.viz_ids:
            p.removeBody(bid, physicsClientId=cid)
        for bid in self.peak_ids:
            p.removeBody(bid, physicsClientId=cid)
        for bid in self.assignment_ids:
            p.removeBody(bid, physicsClientId=cid)
        self.viz_ids.clear()
        self.peak_ids.clear()
...
def print_report(
    ...
    _sm, _ss = divmod(int(sim_time_s), 60)
    _wm, _ws = divmod(int(wall_time_s), 60)
    print(f"  Sim elapsed                   : {_sm}m {_ss:02d}s")
    print(f"  Wall-clock elapsed            : {_wm}m {_ws:02d}s")
    print()
...
# ======================================================================
# End-screen overlay (PyBullet 3D window)
# ======================================================================


def _show_end_screen(
    cid, total_frames, peaks, gt_xy, fw_eval, quad_eval, sim_time_s=0.0, wall_time_s=0.0
):
    """Render a final mission-stats overlay inside the PyBullet 3D window."""
    n_gt = len(gt_xy) if gt_xy else NUM_TARGETS
    n_overlay = max(0, n_gt - NUM_TARGETS)
    backend = (
        "Roboflow plastic-waste detections"
        if _ROBOFLOW_MODEL is not None
        else "HSV cyan color thresholding"
    ...
    # High-contrast colours against a dark panel
    CYAN = [0.30, 0.75, 0.95]
    WHITE = [1.00, 1.00, 1.00]
    LGREY = [0.78, 0.78, 0.82]
    YELLOW = [1.00, 1.00, 0.00]
    GREEN = [0.40, 1.00, 0.40]
    BLUE = [0.50, 0.80, 1.00]

    # ── Build rows: (text, colour, textSize) ──────────────────────────
    rows = [
        ("MISSION COMPLETE", YELLOW, 2.0),
        ("", None, 0),
        (f"FW detection frames        :  {total_frames}", WHITE, 1.3),
        (f"FW detection backend       :  {backend}", WHITE, 1.3),
        (
            f"Ground-truth waste sites   :  {n_gt}  ({NUM_TARGETS} URDF + {n_overlay} overlay)",
            WHITE,
            1.3,
        ),
        (f"Heatmap hotspots detected  :  {len(peaks)}", WHITE, 1.3),
        (
            f"Sim elapsed  :  {_ftime(sim_time_s)}       Wall elapsed  :  {_ftime(wall_time_s)}",
            LGREY,
            1.3,
        ),
    ]
    ...
    if fw_eval is not None:
        ...
        rows.append(
            (
                f"Fixed-wing   AP~mAP={fw_eval['ap_proxy']*100:.1f}%  "
                f"P={fw_eval['precision']*100:.1f}%  "
                f"R={fw_eval['recall']*100:.1f}%",
                GREEN,
                1.3,
    ...
    if quad_eval is not None:
        ...
        rows.append(
            (
                f"Quadcopter   AP~mAP={quad_eval['ap_proxy']*100:.1f}%  "
                f"P={quad_eval['precision']*100:.1f}%  "
                f"R={quad_eval['recall']*100:.1f}%",
                BLUE,
                1.3,
    ...
    rows.append(("", None, 0))
    rows.append(("Press   Q   to exit", YELLOW, 1.5))

    # ── Layout geometry ───────────────────────────────────────────────
    z_top = 38.0
    spacing = 0.50
    total_h = len(rows) * spacing
    mid_z = z_top - total_h / 2.0
    # Shift text left so it's visually centred in the viewport
    text_x = -4.8

    # ── Dark background panel (border + fill) ─────────────────────────
    panel_cx = text_x + 5.0
    border_vis = p.createVisualShape(
        p.GEOM_BOX,
        halfExtents=[7.5, 0.005, total_h / 2.0 + 0.8],
        rgbaColor=[0.18, 0.50, 0.68, 1.0],
        physicsClientId=cid,
    )
    p.createMultiBody(
        baseMass=0,
        baseVisualShapeIndex=border_vis,
        basePosition=[panel_cx, 1.0, mid_z],
        physicsClientId=cid,
    )
    panel_vis = p.createVisualShape(
        p.GEOM_BOX,
        halfExtents=[7.3, 0.01, total_h / 2.0 + 0.55],
        rgbaColor=[0.06, 0.06, 0.10, 1.0],
        physicsClientId=cid,
    )
    p.createMultiBody(
        baseMass=0,
        baseVisualShapeIndex=panel_vis,
        basePosition=[panel_cx, 0.8, mid_z],
        physicsClientId=cid,
    )

    # ── Render text lines (in front of the panel) ─────────────────────
    for i, (text, color, size) in enumerate(rows):
        if not text:
            continue
        p.addUserDebugText(
            text,
            [text_x, 0.0, z_top - i * spacing],
            textColorRGB=color,
            textSize=size,
            physicsClientId=cid,
        )

    # Camera looks horizontally at the panel centre — dark bg fills view
    p.resetDebugVisualizerCamera(
        cameraDistance=10.0,
        cameraYaw=0.0,
        cameraPitch=0.0,
        cameraTargetPosition=[panel_cx, 0.0, mid_z],
        physicsClientId=cid,
    )


def _wait_for_key(cid):
    """Block until any key in PyBullet or any key in the terminal."""
    try:
        import msvcrt
        _has_msvcrt = True
    except ImportError:
...
def _wait_for_exit(cid):
    """Block until Q / ESC in PyBullet or any key in the terminal."""
    try:
        import msvcrt
        _has_msvcrt = True
    except ImportError:
...
# ======================================================================
# Start-screen overlay (PyBullet 3D window)
# ======================================================================


def _show_start_screen(cid, nd, strategy):
    """Show a title card and wait for user to press any key to begin."""
    CYAN   = [0.30, 0.75, 0.95]
    WHITE  = [1.00, 1.00, 1.00]
    LGREY  = [0.65, 0.65, 0.70]
    YELLOW = [1.00, 1.00, 0.00]
    GREEN  = [0.40, 1.00, 0.40]

    backend = (
        "Roboflow plastic-waste"
        if _ROBOFLOW_MODEL is not None
        else "HSV cyan fallback"
    )

    rows = [
        ("Save The Planet 2.0", YELLOW, 2.0),
        ("", None, 0),
        ("Mixed UAV Fleet", WHITE, 1.4),
        ("Waste Detection Mission", WHITE, 1.4),
        ("", None, 0),
        (f"Quadcopters  :  {nd}", LGREY, 1.3),
        (f"Strategy     :  {strategy}", LGREY, 1.3),
        (f"Detection    :  {backend}", LGREY, 1.3),
        ("", None, 0),
        ("Press any key to start", GREEN, 1.5),
    ]
    ...
    border_vis = p.createVisualShape(
        p.GEOM_BOX,
        halfExtents=[5.8, 0.005, total_h / 2.0 + 0.7],
        rgbaColor=[0.18, 0.50, 0.68, 1.0],
        physicsClientId=cid,
    )
    ...
    panel_vis = p.createVisualShape(
        p.GEOM_BOX,
        halfExtents=[5.6, 0.01, total_h / 2.0 + 0.45],
        rgbaColor=[0.06, 0.06, 0.10, 1.0],
        physicsClientId=cid,
    )
    ...
    print("  Press any key in the PyBullet window (or terminal) to start the mission…")
    _wait_for_key(cid)

    # Clean up so the title card doesn't linger during the mission
    for tid in text_ids:
        p.removeUserDebugItem(tid, physicsClientId=cid)
    p.removeBody(panel_id, physicsClientId=cid)
    p.removeBody(border_id, physicsClientId=cid)
...
def run(
    ...
    heatmap_display_steps = int(15.0 * ctrl_hz)
    heatmap_display_ctr = -1
    ...
    if gui:
        _show_start_screen(cid, nd, strategy)
        _mission_wall_t0 = time.time()
    ...
    try:
        while True:
            ...
            # ── FW_LAND ──────────────────────────────────────────────
                ...
                if fw_land_step >= fw_land_max and heatmap_display_ctr < 0:
                    # ── First frame after landing: analyse & render heatmap ──
                    ...
                    heatmap_display_ctr = 0
                    total_assigned = sum(len(a) for a in assignments)
                    print(f"  [Heatmap] Displaying for 15 s before quad departure …")

                elif fw_land_step >= fw_land_max and heatmap_display_ctr >= 0:
                    # ── Hold: quads hover while heatmap is visible ──
                    heatmap_display_ctr += 1
                    if heatmap_display_ctr >= heatmap_display_steps:
                        heatmap.remove_viz(cid)
                        squad_go_step = 0
            ...
            # ── QUAD_LAND ────────────────────────────────────────────
                ...
                if quad_land_step >= quad_land_max:
                    ...
                    print_report(
                        ...
                        gt_xy=gt_xy,
                        sim_time_s=step * dt,
                        wall_time_s=time.time() - _mission_wall_t0,
                    )
                    mission_state = DONE
                    break
        ...
        # ── End-screen: show stats overlay & hold window open ──────────
        _end_sim_s = step * dt
        _end_wall_s = time.time() - _mission_wall_t0
        if gui:
            try:
                _es_peaks = heatmap.find_peaks()
                _show_end_screen(
                    cid,
                    total_frames,
                    _es_peaks,
                    ...
                    fw_eval,
                    quad_eval,
                    sim_time_s=_end_sim_s,
                    wall_time_s=_end_wall_s,
                )
                print(
                    "\n  Press Q in the PyBullet window (or any key in terminal) to exit…"
                )
                _wait_for_exit(cid)
            except Exception:
                pass
        ...
        try:
            ...
            # ── Run tracker: CSV + JSON + overlay + running averages ──
            _bat_used = [init_bat_pct[i] - final_bat_pct[i] for i in range(nd)]
            tracker = RunTracker()
            _row = tracker.build_row(
                strategy=strategy,
                num_drones=nd,
                seed=42,
                fw_eval=fw_eval,
                quad_eval=quad_eval,
                heatmap_peaks=len(peaks_for_report),
                heatmap_tp=_tp,
                heatmap_fp=_fp,
                heatmap_precision=_prec,
                heatmap_recall=_rec,
                gt_items=len(report_gt_xy),
                fw_frames=total_frames,
                fw_detections=total_px,
                photos_taken=photos_taken,
                battery_used_pct=_bat_used,
                sim_time_s=_sim_elapsed_s,
                wall_time_s=_wall_elapsed_s,
                flight_times_s=_flight_times,
                crypto_total_msgs=sum(comms.message_count),
                crypto_total_mj=sum(comms.millijoules_per_drone()),
            )
            tracker.record_run(_row)
            tracker.print_running_averages()
